...
exports.run = (client, message, args, tools) => {
    ...
    const patEmb = new Discord.RichEmbed()
        ...
        .setDescription(`<@${message.author.id}> **выстрелил(а) в** <@${message.author.id}>.. **Ой, подожди! Вы не можете выстрелить в себя!**`)
        .setColor(0xA901DB)
        .setImage(images);
        if (!args[0]) {
    ...
    if (!message.mentions.users.first()) return message.channel.send(`Пожалуйста, укажите в кого вы хотите выстрелить!!!`).then(msg => {
        msg.delete(3000)
    });
